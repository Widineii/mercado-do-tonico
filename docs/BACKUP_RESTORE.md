...
1. Feche o sistema.
2. Execute `BACKUP_MERCADO_TONICO.bat`.
3. O arquivo sera salvo em `backups\`.

## Restore

1. Feche o sistema.
2. Execute `RESTORE_MERCADO_TONICO.bat` para restaurar o backup mais recente automaticamente.
3. Se preferir manual, copie o backup desejado para `data\mercado-tonico.db`.
4. Reabra o sistema.

## Teste recomendado (5 minutos)

1. Faça um backup com `BACKUP_MERCADO_TONICO.bat`.
2. Renomeie temporariamente `data\mercado-tonico.db` para `data\mercado-tonico_teste.db`.
3. Rode `RESTORE_MERCADO_TONICO.bat`.
